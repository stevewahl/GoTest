...
func hashpostreq(rw http.ResponseWriter, req *http.Request) {
    ...
    if len(pw) > 0 {
        ...
        log.Println("clear passwod: " + pw + "  hashed password: " + pwhash)
...
func shutsetreq(rw http.ResponseWriter, req *http.Request) {
    req.ParseForm()
    if req.Method != "PUT" {
        http.Error(rw, "ERROR in request to /shutdown. Must be PUT", 400)
    ...
    cntmut.Unlock()
    if cnt == 0 {
        http.Error(rw, "Server no longer accepting new requests and exiting.", 200)
        log.Println("Password server exiting")
        time.After(time.Millisecond + 2000)
        os.Exit(0)
    }
    log.Println("Server not accepting new requests at this time.")
    http.Error(rw, "Server is now no longer accepting new requests.", 200)
}
